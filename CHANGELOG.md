...
## [0.9.0] - 2026-09-18
### Added
- `thatch({ resurrectSessions: true })` (LIBS-7): opt-in server-side session resurrection, for the standalone notification stream's own GET reconnect only. Off by default, a GET carrying an `mcp-session-id` thatch doesn't recognize — e.g. every id after a restart — gets 404, forcing the client through a full re-`initialize` before its next request succeeds, and its background notification stream stays dead until then. With it on, such a GET instead re-creates the session under that same id: the `auth` hook re-runs against the NEW request (a rejection still answers 401, and never resurrects), and the resurrected connection's `headers` come only from that new request — never the old, gone connection's. `connect` fires again, same as a fresh connection. A POST or DELETE under an unrecognized id still 404s even with this on — deliberately: the MCP SDK client (which Claude Code embeds) only ever reopens its standalone stream right after its own `initialize` handshake, so resurrecting a POST would "succeed" with no re-initialize and thus no stream ever reopening, leaving the session registered but permanently deaf. Measured against that real client: it retries its dropped GET automatically, with the original session id and headers, for a couple of seconds (2 attempts, default backoff) before giving up for good — resurrection covers a restart that completes inside that window with no client-side change at all; a longer restart falls back to today's 404 → re-initialize → fresh stream path, unchanged. See `README.md`'s restart-behaviour section.
